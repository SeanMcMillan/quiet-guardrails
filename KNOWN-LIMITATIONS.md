...
- **`git config` code-exec isn't gated — but it prompts anyway.** `git config core.hooksPath …` and alias `!`-commands can run arbitrary code, and the guard doesn't gate `git config` (doing so would also prompt on read-only `--get`/`--list`). It doesn't need to: Claude Code does **not** read-only-classify `git config` *writes* — a `git config <key> <value>` raises the normal permission prompt on its own (observed against 2.1.x), so an ungated config write fails safe rather than slipping through.
